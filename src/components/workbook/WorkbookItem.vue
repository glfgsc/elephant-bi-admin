<template> 
    ...
    >
        ...
        >
            <v-card-text>
                <div style="z-index:1" @click="$emit('getWorkbooks',{parentId: props.workbook.id,name: props.workbook.name,sortColumn:sortColumn})">
                    <v-img src="/static/img/folder.jpg"  height="50" v-if="props.workbook.type==0">
                    </v-img>
                    <v-img src="/static/img/sql.jpg" height="50" v-if="props.workbook.type==1&&props.workbook.fileType==0"></v-img>
                    <v-img src="/static/img/excel.jpg" height="50" v-if="props.workbook.type==1&&props.workbook.fileType==1"></v-img>
                </div>
                <div 
                ...
                >
                    <div class="d-flex flex-row justify-center align-center pl-1 pr-1 pt-1 pb-1" v-if="isUpdate">
                        <el-icon size="small" color="#409EFF" class="cursor-pointer mr-2" @click="rename(ruleFormRef)">
            ...
            <v-card-actions class="d-flex flex-column justify-center align-center">
                <div v-if="isUpdate">
                    <el-form 
                    ref="updateWorkbookRef" 
                    :model="changedWorkbook" 
                    :rules="rules" 
                    @submit.native.prevent>
                        <el-form-item prop="name">
                            ...
                            size="small"  
                            id="rename"
                            @keydown.enter="rename(updateWorkbookRef)">
                            </el-input>
                        </el-form-item>
                    </el-form>
                ...
                @click="$emit('getWorkbooks',{parentId: props.workbook.id,name: props.workbook.name,sortColumn: sortColumn})">
                    <el-dropdown>
                        <span class="text-caption workbook-name">{{ props.workbook.name }}</span>
                        <template #dropdown>
                            <el-dropdown-menu>
                                <el-dropdown-item>
...
import {updateWorkbook,checkName} from '@/api/workbook'
...
const {sortColumn} = storeToRefs(datasetStore)
const updateWorkbookRef = ref<FormInstance>()
...
async function validateName(rule:any,value:any,callback:any){
    ...
    try{
        let res = await checkName({id: props.workbook.id,name: value,parentId: props.workbook.parentId})
        if(!res.data){
...
async function rename(formEl:FormInstance | undefined){
    if (formEl){
        await formEl.validate().then(async (valid: any)=>{
            if (valid) {
                try{
                    ...
                    if(res.code == 200){
                        await asyncGetWorkbooks({parentId: props.workbook.parentId,sortColumn:sortColumn.value})      
